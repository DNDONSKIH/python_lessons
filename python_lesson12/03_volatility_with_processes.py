...
def time_track(func):
    def surrogate(*args, **kwargs):
        ...
        print(f'Функция работала {elapsed} секунд(ы)', 'process id:', os.getpid())
        return result

    return surrogate


class Volatile_calculator(multiprocessing.Process):

    def __init__(self, filename, connection, *args, **kwargs):
    ...
    def run(self):
        ...
        self.connection.put((self.ticker, self.volatile))
...
def calc_tickers():
    ...
    for dirpath, dirnames, filenames in os.walk(filepath):
        if filenames:
            for file in filenames:
                ...
                processes.append(Volatile_calculator(full_file_path, connection=dataQueue))
    ...
    while not dataQueue.empty():
        _ticker, _volatile = dataQueue.get()
        ticker_dict[_ticker] = _volatile
